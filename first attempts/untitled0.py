# -*- coding: utf-8 -*-
"""
Created on Fri Jul 12 09:23:32 2019
...
from source.sorct1_gamma import *
from sklearn.model_selection import KFold
from source.prova_extendedSCHIFO1 import *


data = pd.read_csv('sonar.csv',sep=';')
data_std = data.copy()
...
data_std = data_std.sample(frac= 1 ,random_state = 65).reset_index(drop=True)
...
fin_ex = {'simple':[], 'l1':[], 'l_inf':[], 'l0':[],'l0_inf':[]}
fin = {'simple':{}, 'l1':{}, 'l_inf':{}, 'l0':{},'l0_inf':{}}
for i in fin:
        for j in range(iterations):
            fin[i][j] = []

print("It will start soon")
for p in range(5):
    initialization = init[p]
    
    acc_ex = {'simple':[], 'l1':[], 'l_inf':[], 'l0':[],'l0_inf':[]}
    acc = {'simple':{}, 'l1':{}, 'l_inf':{}, 'l0':{},'l0_inf':{}}
    for i in acc:
        for j in range(iterations):
            acc[i][j] = []
    ...
    for train_indices, test_indices in k_fold.split(data_std):
        ...
        for i in acc_ex:
            model = SORCT1(df_train_ex2,I_in_k,I_k)
            model.set_init(ini)
            model.createModel()
            model.set_alpha(5)
            if i=='l1' or i=='l0':
                lam = lambdas
            elif i=='l_inf' or 'l0_Inf':
                lam = l
            else:
                lam = 0
