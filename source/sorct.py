...
class ORCT_2:
    ...
    def createModel(self):
        ...
        def global_ma(model,f,tb):
            return model.beta[f]>= -model.a[f,tb]
        self.model.globalma = Constraint(self.model.f_s, self.model.N_B, rule=global_ma)
    ...
    def objective(self,l_global=0,l_local=0):
        self.model.l_global = l_global
        self.model.l_local = l_local
        def cost_rule(model):
            return sum( sum( sum( model.P[i,t]* sum(model.W[k,j]*model.C[j,t] for j in self.model.K if k!=j)  for t in self.model.N_L) for i in self.model.I_k[k] ) for k in self.model.K )+self.model.l_global*(sum(model.beta[j] for j in self.model.f_s))+self.model.l_local*(sum(sum(model.a_plus[j,tb]+model.a_minus[j,tb] for tb in self.model.N_B)for j in self.model.f_s))
