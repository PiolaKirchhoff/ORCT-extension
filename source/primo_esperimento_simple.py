# -*- coding: utf-8 -*-
"""
Created on Fri Jul 19 18:37:13 2019
...
for p in range(5):
    ...
    for train_indices, test_indices in k_fold.split(data_std):
        ...
        for i in acc:
            ...
            delta_l[i].append(model.delta_l())
            delta_g[i].append(model.delta_g())

    for i in acc:
        fin[i].append(np.mean(acc[i]))
        fin_dl[i].append(np.mean(fin_dl[i]))
        fin_dg[i].append(np.mean(fin_dg[i]))
...
print("Delta local:")
for i in fin:
    print(np.mean(fin_dl[i]))

print("Delta global:")
for i in fin:
    print(np.mean(fin_dg[i]))    
